    ...
    var_dump($_POST);
        if( isset($_POST['username'])
            ...
            try{
                ...
                die("Table couldn't be completed, oboarding process failed."); 
            }
            //save the new user in the database. 
            // ONLY IF THERE IS NO EXISTING ADMIN USER!!!
            $preflight_check_query = "SELECT count(*) AS count FROM userdata WHERE userdata.role='Admin'"; 
            $result = $pdo->query($preflight_check_query)->fetch(PDO::FETCH_ASSOC);
            $count = $result['count'];
            if(!(boolval($count))){
                try{
                    $query = "INSERT INTO 
                        userdata (logon_attempts, mail, username, password, role, wd_property_preferences)
                        VALUES 
                        (0, :mail, :username, :password, 'Admin', '') "; 
                    $data = array(
                        'mail' => $_POST['email'], 
                        'username' => $_POST[''], 
                ...
                throw new Exception("One or more Admin users are already in the datase; the onboarding process does  not allow to create more than one! Log in using the existing Admin-account and add an Admin-user manually.");
            ...
            die("Don't even continue with this!"); 
        }else{
